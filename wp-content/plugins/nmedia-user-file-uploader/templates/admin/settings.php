...
//filemanager_pa($nmfilemanager -> plugin_settings);
...
<div id="filemanager-tabs" class="tab-container">
	...
	<div id="<?php echo $id?>" class="general-settings">
		...
		<ul>
			<?php 
			if( isset($options['meat']) && is_array($options['meat']) ) {
			...
			foreach($options['meat'] as $key => $data){
				...
				$the_value = ( isset($nmfilemanager -> plugin_settings[ $data['id'] ]) ? $nmfilemanager -> plugin_settings[ $data['id'] ] : $default_value);
				?>
			...
			<?php switch($data['type']){
				...
				<ul>
					...
					<!-- setting value -->
					<script>
					setChecked('<?php echo $data['id']?>', '<?php echo json_encode($nmfilemanager -> plugin_settings[ $data['id'] ])?>');
					</script>
				</ul>
				...
				case 'radio':?>
								<ul>
									...
									<script>
									setCheckedRadio('<?php echo $data['id']?>', '<?php echo $nmfilemanager -> plugin_settings[ $data['id'] ]?>');
									</script>
								</ul>
									...
									<h4><?php echo $data['desc']?> </h4>
										...
										<select name="<?php echo $data['id']?>" id="<?php echo $data['id']?>">
											...
											<?php foreach($data['options'] as $k => $label){
													...
													$selected = ($k == $nmfilemanager -> plugin_settings[ $data['id'] ]) ? 'selected = "selected"' : '';
